...
async function subPOD(e, t){
  ...
  // dataApi.addPod() writes to /pod/{date}/{pushId} on Firebase, so all
  // devices see the entry within milliseconds via the listenPodToday listener.
  try {
    await dataApi.addPod({
      ...
      notes:      nt,
      date:       date,
      // The byName field on the server defaults to 'Anonymous' if no
      // user is unlocked — but if they typed their name in the form,
      // surface that explicitly so the audit trail is meaningful.
    });
    // If the submitted date is not today, ensure a live listener exists for
    // that date so all devices (including this one) receive the new entry.
    if (date !== dataApi.todayISO() && typeof _attachPodDateListener === 'function') {
      _attachPodDateListener(date);
    }
    // Also push a Daily Progress feed entry — this is what the DPR view bullets.
    await dataApi.addDailyProgress({
      module:   ({s:'Solar',w:'WTG',l:'Land',b:'BOP'})[t] || t,
      activity: actLabel,
      qty:      qty,
      unit:     '',
      remarks:  by ? ('by ' + by + (nt ? ' · ' + nt : '')) : nt
    });
    cov('pov');
