...
function _rndrOnePathway(t, which, title, subtitle){
  ...
  const stageRows = pw.stages.map((s,i)=>`
    <div style="display:flex;align-items:center;gap:8px;padding:5px 7px;border-bottom:1px solid var(--b1);">
      <input type="checkbox" ${s.done?'checked':''} ${disAttr}
             onchange="wtgSavePathStage('${t.id}','${which}',${i},'done',this.checked)"
             style="cursor:${ed?'pointer':'not-allowed'};">
      ...
      <input type="text" value="${(s.remarks||'').replace(/"/g,'&quot;')}" ${disAttr}
             placeholder="remarks"
    ...
    ${open?`<div class="wtg-section-body">
      ...
      ${ed?`
        ...
        <div style="display:flex;justify-content:flex-end;margin-top:6px;">
          <button class="btn btwt bts" style="font-size:9px;padding:4px 10px;" onclick="event.stopPropagation();wtgOpenAddActivity('${sectionKey}')">+ Add Activity</button>
        </div>
      `:''}
    </div>`:''}
  </div>`;
...
function wtgOpenAddActivity(sectionKey){
  if(!_isWtgEditor()){
    if(typeof showToast==='function') showToast('🔒 WTG Engineer login required','er');
    return;
  }
  ...
  host.innerHTML = `
    <div style="background:var(--card2);border:1px dashed var(--ac);border-radius:6px;padding:10px;margin:8px 0;">
      <div style="font-weight:700;font-size:10px;margin-bottom:6px;color:var(--ac);">+ New Activity (applies to all turbines)</div>
      ...
      <div style="display:flex;gap:6px;margin-top:8px;">
        <button class="btn btwt bts" style="font-size:9px;padding:4px 10px;" onclick="wtgSubmitNewActivity('${sectionKey}')">✓ Add Activity</button>
...
async function wtgSubmitNewActivity(sectionKey){
  ...
  if(typeof wtgAddCustomActivity !== 'function'){
    if(typeof showToast==='function') showToast('❌ Activity API unavailable','er');
    return;
  }
  const newKey = wtgAddCustomActivity(sectionKey, name, subs);
  if(!newKey){ if(typeof showToast==='function') showToast('❌ Failed to add','er'); return; }
  // Persist customActs + each turbine's seeded acts
  await _wtgPersistCustomActs();
  if(typeof showToast==='function') showToast('✅ Activity "'+name+'" added to all turbines','ok');
  // Open the new activity so the user can immediately edit
  _wtgOpenSection = sectionKey;
  _wtgOpenAct = newKey;
  if(_wtgSelected && _wtgSelected!=='ZP') selectTurbine(_wtgSelected);
}
...
function wtgOpenAddSub(sectionKey, actKey){
  ...
  host.innerHTML = `
    <div style="background:var(--card2);border:1px dashed var(--ac);border-radius:6px;padding:8px;margin:6px 0;">
      <label style="font-size:8px;color:var(--t3);display:block;">New Sub-activity Name (applies to all turbines)
        <input id="wtg-newsub-name-${sectionKey}-${actKey}" type="text" placeholder="e.g. Final inspection"
          style="display:block;width:100%;margin-top:3px;background:var(--card);border:1px solid var(--b1);border-radius:3px;color:var(--t1);padding:4px 6px;font-size:10px;">
      </label>
      <div style="display:flex;gap:6px;margin-top:6px;">
        <button class="btn btwt bts" style="font-size:9px;padding:4px 10px;" onclick="wtgSubmitNewSub('${sectionKey}','${actKey}')">✓ Add</button>
...
async function wtgSubmitNewSub(sectionKey, actKey){
