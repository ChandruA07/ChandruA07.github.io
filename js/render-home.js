...
function _wtModuleIcon(m){
  ...
  if (m==='l') return (typeof _landImg === 'function') ? _landImg(13) : '🌍';
  return '📍';
}
...
function renderHomeWorkTable(){
  ...
  const ndByMod = {s:[], w:[], b:[], l:[]};
  nextDay.forEach(p => { if (ndByMod[p._mod]) ndByMod[p._mod].push(p); });

  const modLabels = {s:'Solar', w:'WTG', b:'BOP', l:'Land'};
  const modNav    = {s:'solar', w:'wtg',  b:'bop', l:'land'};
  const modCol    = {s:'var(--sol)', w:'var(--wtg)', b:'var(--bop)', l:'var(--land,var(--ok))'};
      ...
      return `<div class="wt-modhdr ${m}">${_wtModuleIcon(m)} ${modLabels[m]}</div>
        <ul class="wt-pod-ul">
          ${Object.entries(byLoc).map(([loc, items]) => items.map(p => {
            const act = esc(_wtPodActivity(p));
            const qty = p.qty != null && +p.qty > 0 ? ` – target ${esc(p.qty)}` : '';
            return `<li><span class="wt-loc">${esc(loc)}</span> <b>${act}</b>${qty}</li>`;
          }).join('')).join('')}
        </ul>`;
    })() : `<div class="wt-modhdr ${m}">${_wtModuleIcon(m)} ${modLabels[m]}</div>
        ...
        rowsHtml.push(`
          <div class="wt-live-row">
            ...
            <div style="flex:1;min-width:0;">
              <div style="font-size:10px;color:var(--t1);"><b>${loc?esc(loc)+': ':''}</b>${esc(act)}</div>
      ...
      return all.map(p => {
        ...
        return `
          <div class="wt-pending-row">
            <div style="font-size:9px;color:var(--t2);">${loc?'<b style="color:var(--t1);">'+esc(loc)+'</b>: ':''}${esc(act)}</div>
    ...
    const ndCellHtml = ndRows.length
      ? (() => {
          ...
          return `<ul class="wt-nextday-ul">
            ${Object.entries(byLoc).map(([loc, items]) => items.map(p => {
              const act = esc(_wtPodActivity(p));
              const qty = p.qty != null && +p.qty > 0 ? ` – ${esc(p.qty)}` : '';
              return `<li><span class="wt-loc">${esc(loc)}</span> <b>${act}</b>${qty}</li>`;
            }).join('')).join('')}
          </ul>`;
        })()
...
function rndrHome(){
  ...
  // Total manpower must reflect TODAY's actual POD submissions, not the static
  // seed values in DB.mp (which never change). Sum the `mp` field of every POD
  // entry dated today across all modules. The home view re-renders whenever
  // POD data changes (bridge), so this stays live.
  const _mpTodayIso = (typeof dataApi!=='undefined' && dataApi.todayISO)
    ? dataApi.todayISO() : new Date().toISOString().slice(0,10);
  let totalMp = 0;
  ['s','w','b','l'].forEach(m=>{
    (DB.pod[m]||[]).forEach(p=>{
      if((p.date||_mpTodayIso)===_mpTodayIso) totalMp += parseFloat(p.mp)||0;
    });
  });
  ...
  let _sol=sol,_wtg=wtg;
  if(!_live && _snap){
    if(_snap.solar) _sol=_snap.solar.overall;
    if(_snap.wtg)   _wtg=_snap.wtg.overall;
  }
  // Overall reflects Solar + WTG (MW-weighted) plus BOP, which now has real
  // feeder/pole/span tracking. Land remains "Coming Soon" until its detailed
  // tracker ships.
  const _bop = (typeof calcBopProg === 'function') ? Math.round(calcBopProg()) : 0;
  const _tot = Math.round((_sol*70.4 + _wtg*70.2) / (70.4 + 70.2));

  // ── ROW 1: SQUARE KPI CARDS ──────────────────────────────────────────────
  document.getElementById('home-kpi-row').innerHTML=[
    {label:'Overall Progress',val:_tot+'%',       sub:'Solar + WTG · MW-weighted',col:'var(--ac)',  icon:'continuumlogo.png',nav:'home',  tip:'Overall project progress (Solar + WTG, MW-weighted). Land is not yet tracked at the detail level.'},
    {label:'Land Progress',   val:'Coming Soon',  sub:'Detail tracker WIP',col:'var(--land)',icon:'land.png',          nav:'land',  tip:'Detailed Land activity tracking is coming in a future release.'},
    ...
    {label:'BOP Progress',    val:_bop+'%',        sub:'33/66kV · PSS · GSS',col:'var(--bop)', icon:'bop.png',       nav:'bop',   tip:'BOP progress — 33kV & 66kV lines, PSS and GSS (live from feeder/pole/span data).'},
  ...
  // Header notification marquee — smooth scrolling project updates
  if(typeof renderHeroMarquee==='function') renderHeroMarquee();
}

// ═══════════════════════════════════════════════════════════
//  HEADER NOTIFICATION MARQUEE — smooth scrolling project updates
// ═══════════════════════════════════════════════════════════
// Populates the header ribbon with the latest project updates: live
// notifications when present, otherwise recent POD submissions and key
// progress figures. Items scroll horizontally (CSS marquee), duplicated
// so the loop is seamless. Re-runs on every home render so it stays live.
const _HMQ_MODCOL = { solar:'#ffca28', wtg:'#4fc3f7', bop:'#ab47bc', land:'#00c853', hse:'#ff5252', general:'#8aaccf' };
function _heroMarqueeItems(){
  const items=[];
  // 1) live notifications (via notify's POD fallback shape) if available
  try{
    const today=(typeof dataApi!=='undefined'&&dataApi.todayISO)?dataApi.todayISO():new Date().toISOString().slice(0,10);
    const modMap={s:'solar',w:'wtg',b:'bop',l:'land'};
    Object.keys(modMap).forEach(k=>{
      (DB.pod[k]||[]).filter(p=>(p.date||today)===today).slice(-6).forEach(p=>{
        items.push({mod:modMap[k],text:(p.activity||'Activity')+(p.qty?(' · qty '+p.qty):''),by:p.by||'Site Engineer'});
      });
    });
  }catch(e){}
  // 2) key progress figures as ambient updates
  try{
    const s=Math.round(calcSolarProg()), w=Math.round(calcWtgProg()), b=Math.round(calcBopProg());
    items.push({mod:'solar',text:'Solar progress at '+s+'%',by:'Live'});
    items.push({mod:'wtg',text:'WTG progress at '+w+'%',by:'Live'});
    items.push({mod:'bop',text:'BOP progress at '+b+'%',by:'Live'});
  }catch(e){}
  // 3) open ROW issues callout
  try{
    const openRow=(DB.rowIssues||[]).filter(r=>(r.status||'Open')==='Open').length;
    if(openRow>0) items.push({mod:'hse',text:openRow+' open ROW issue'+(openRow>1?'s':'')+' need attention',by:'Alert'});
  }catch(e){}
  if(!items.length) items.push({mod:'general',text:'Welcome to the SWPPL 140 MW Hybrid EPC Dashboard — updates will appear here as work is logged',by:'System'});
  return items;
}
function renderHeroMarquee(){
  const track=document.getElementById('hero-marquee-track');
  if(!track) return;
  const items=_heroMarqueeItems();
  const ICO={solar:'☀️',wtg:'🌀',bop:'⚡',land:'🌍',hse:'⚠️',general:'📢'};
  const chip=(it)=>{
    const m=(it.mod||'general').toLowerCase();
    const col=_HMQ_MODCOL[m]||_HMQ_MODCOL.general;
    return `<span class="hmq-item">
      <span class="hmq-ic">${ICO[m]||'📢'}</span>
      <span class="hmq-tag" style="color:${col};border-color:${col};">${esc((it.mod||'').toUpperCase())}</span>
      <span class="hmq-text">${esc(it.text)}</span>
      <span class="hmq-by">— ${esc(it.by)}</span>
    </span>`;
  };
  const seq=items.map(chip).join('');
  // duplicate the sequence so the horizontal loop is seamless
  track.innerHTML=seq+seq;
  // adjust animation duration to content length for a steady, readable speed
  const dur=Math.max(20, Math.min(75, items.length*8));
...
function renderRowTracker(){
  ...
  const rows=issues.slice().sort((a,b)=>{
    ...
    return `<tr style="cursor:pointer;" onclick="openRowEdit('${id}')" data-tt="Click to edit / close this ROW">
      <td><b style="color:var(--t1);">${esc(r.loc)}</b><div style="font-size:7px;color:var(--t3);">${esc(r.type)||'WTG'}</div></td>
...
function _openRowAddImpl(){
  ...
  document.getElementById('p-b').innerHTML=`
    <div class="al al-w" style="margin-bottom:9px;">⚠️ Right-of-Way (ROW) issues block construction. Logged in as <b>${esc(cu.name)}</b> (${esc(cu.role)}).</div>
...
function openRowEdit(idOrLoc){
  ...
  document.getElementById('p-b').innerHTML=`
    <div class="al ${r.status==='Closed'?'al-s':'al-w'}" style="margin-bottom:9px;">
      <b>${esc(r.loc)}</b> — ${esc(r.type)||'WTG'} • Open ${days} days
    </div>
    <div class="fg">
      <label class="fl">Issue / Reason</label>
      <textarea class="fta" id="row-edit-issue" style="min-height:64px;">${esc(r.issue||'')}</textarea>
    </div>
    <div class="fr">
      <div class="fg">
        <label class="fl">Status</label>
         ...
         <div style="background:var(--card2);border-left:3px solid ${p._col};border-radius:6px;padding:9px 12px;">
           <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:4px;">
             <div style="display:flex;align-items:center;gap:6px;flex:1;min-width:0;">
               ...
               <b style="color:var(--t1);font-size:11px;">${esc(p.activity)||'—'}</b>
             </div>
           ...
           <div style="display:flex;gap:10px;flex-wrap:wrap;font-size:9px;color:var(--t2);">
             ...
             <span>👤 By: <b>${esc(p.by)||'—'}</b></span>
             ${p.contractor?`<span>🏗️ Contractor: <b>${esc(p.contractor)}</b></span>`:''}
           </div>
           ${p.notes?`<div style="margin-top:5px;padding:5px 7px;background:var(--card3);border-radius:4px;font-size:9px;color:var(--t3);">📝 ${esc(p.notes)}</div>`:''}
         </div>`).join('')}
       </div>`;
...
function openDailyProgressPopup(){
  ...
  function _renderBullet(e) {
    ...
    const sub  = e.sub ? ` · ${esc(e.sub)}` : '';
    const who  = e.byName ? ` · 👤 ${esc(e.byName)}` : '';
...
function _openMilestoneFormImpl(){
  ...
  document.getElementById('p-b').innerHTML=`
    <div class="al al-w" style="margin-bottom:9px;">Logged in as <b>${esc(cu.name)}</b> (${esc(cu.role)})</div>
...
function applyScope(){
  ...
  // Targeted Firebase writes — no bulk saveDB. Solar meta carries the
  // totals; per-ITC MW goes to solar/meta/itcMW; remaining scope fields
  // are persisted as leaf updates so other devices pick them up live.
  (async () => {
    try{
      const itcMW = {};
      Object.keys(DB.solar.itcs).forEach(id => { itcMW[id] = DB.solar.itcs[id].mw; });
      await dataApi.updateSolarMeta({ totalMW: DB.solar.totalMW, itcMW });
      // WTG + 66kV scope — leaf writes (debounced helper keeps it cheap)
      dataApi.debouncedUpdate('wtg/meta/totalMW', DB.wtg.totalMW);
      dataApi.debouncedUpdate('wtg/meta/count',   DB.wtg.count);
      dataApi.debouncedUpdate('bop/meta/towers66', DB.bop66.totalTowers);
      if (DB.bop66.feeders[0]) dataApi.debouncedUpdate('bop/meta/km66', DB.bop66.feeders[0].km);
      showToast('✅ Scope updated & synced!','ok');
    }catch(err){
      showToast('❌ '+(err.message||'Scope save failed'),'er');
    }
  })();
  cov('pov');rndrHome();
}
