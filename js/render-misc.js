    ...
    <div class="pnl" style="position:relative;">
      ...
      ${entries.length
        ? entries.slice().reverse().map(x=>{
            const status = x.status || 'nys';
            const chipCls = (typeof _statusChipClass === 'function') ? _statusChipClass(status) : 'wt-status-nys';
            const lbl = (typeof _statusLabel === 'function') ? _statusLabel(status) : 'Not Yet Started';
            return `
          <div style="background:var(--card3);border:1px solid var(--b2);border-radius:8px;padding:10px;margin-bottom:7px;font-size:10px;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;gap:6px;">
              ...
              <span class="wt-status-chip ${chipCls}">${lbl}</span>
            </div>
            <div class="mst">
              ...
              ${x.progress != null && +x.progress > 0 ? `<div class="ms">Progress: <b>${x.progress}</b></div>` : ''}
            </div>
            ${x.notes?`<div style="color:var(--t3);margin-top:3px;font-size:9px;">📝 ${x.notes}</div>`:''}
            ${(Array.isArray(x.resources)&&x.resources.length)?`<div style="color:var(--t2);margin-top:4px;font-size:9px;">🔧 ${x.resources.map(r=>r.type+' × <b>'+r.qty+'</b>').join(' · ')}</div>`:''}
            ${x.remark?`<div style="color:var(--wn);margin-top:3px;font-size:9px;">💬 ${x.remark}</div>`:''}
          </div>`;
          }).join('')
...
async function submitHSEObservation(){
  ...
  try {
    let photoURL = null;
    if (photoFile && typeof storage !== 'undefined' && storage.uploadHseImage) {
      const result = await storage.uploadHseImage(photoFile);
      photoURL = result.url;
    }
