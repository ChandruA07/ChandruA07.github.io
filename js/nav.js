...
function setBc(v,p){
  const L={home:'Dashboard',solar:'☀️ Solar',wtg:'🌬️ WTG',land:'🌍 Land',landwtg:'📍 WTG Land',landsol:'🌾 Solar Land',bop:'⚙️ BOP','bop-33kv':'33kV','bop-66kv':'66kV','bop-pss':'PSS','bop-gss':'GSS',pod:'📋 POD',dpr:'📝 DPR',safety:'🦺 Safety',manpower:'👷 Manpower',map:'🗺️ Map',itc:'ITC Detail'};
  ...
  if(bc)bc.innerHTML=h;
}

function ldr(on){const l=document.getElementById('ldr');if(!l)return;l.style.width=on?'72%':'100%';if(!on)setTimeout(()=>l.style.width='0',220);}

function rndr(v,p){
  const m={
    home:rndrHome,
    solar:rndrSolar,
    itc:()=>rndrITC(p.itc||curITC),
    wtg:()=>{rndrWtg();wTab(curWT);},
    land:rndrLand,
    landwtg:rndrWtgLand,
    landsol:rndrSolLand,
    bop:rndrBop,
