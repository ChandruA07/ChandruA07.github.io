...
function rndrWtgLandTable(){
  ...
  <tbody>${DB.wtgLand.locs.map(l=>{const p=calcWtgLandLocProg(l);const done=l.stages.filter(Boolean).length;return`<tr>
    <td><b>${esc(l.id)}</b></td><td>${esc(l.svy)}</td><td>${esc(l.pa)}</td><td>${esc(l.la)}</td>
...
function showWtgLandDet(id){
  ...
  document.getElementById('lm-b').innerHTML=`
    ...
    <div class="ir"><div class="irl">Survey No.</div><div class="irr">${esc(l.svy)}</div></div>
    ...
    <div class="ir"><div class="irl">Compensation</div><div class="irr">${esc(l.comp)}</div></div>
    <div class="sep"></div>
    ...
    <div class="fg" style="margin-top:10px;"><label class="fl">Notes</label><textarea class="fta" id="ln-${id}" placeholder="Update notes...">${esc(l.notes||'')}</textarea></div>
...
async function removeWtgLand(id){
  ...
  try {
    await dataApi.deleteWtgLandLoc(id);
...
function rndrLeaseTable(){
  ...
  <tbody>${all.length?all.map(l=>`<tr><td><b>${esc(l.bid)}</b></td><td>${esc(l.own)}</td><td>${esc(l.svy)}</td><td>${esc(l.dur)}</td>
    <td><span class="chip ${l.ls==='Executed'?'cg':l.ls==='In Progress'?'cy':'cr'}">${esc(l.ls)}</span></td>
    <td><span class="chip ${l.doc==='Complete'?'cg':'cy'}">${esc(l.doc)}</span></td>
    <td><span class="chip ${l.reg==='Done'?'cg':'cy'}">${esc(l.reg)}</span></td>
    <td>${esc(l.rem)}</td>
...
async function submitAddLease(e){
  e.preventDefault();
  const blk = document.getElementById('al-blk').value;
  const lease = {
    own: document.getElementById('al-own').value,
    svy: document.getElementById('al-sv').value,
    dur: document.getElementById('al-dur').value,
    ls:  document.getElementById('al-ls').value,
    doc: document.getElementById('al-ds').value,
    reg: document.getElementById('al-rs').value,
    rem: document.getElementById('al-rm').value
  };
  try{
    // Write to Firebase — the land listener mirrors it back into
    // DB.solLand.blocks[blk].leases and re-renders automatically,
    // so the entry survives refresh and appears on every device.
    await dataApi.addSolLease(blk, lease);
    // Optimistic local echo so the table updates instantly even
    // before the round-trip completes.
    if (DB.solLand.blocks[blk]) {
      DB.solLand.blocks[blk].leases = DB.solLand.blocks[blk].leases || [];
      DB.solLand.blocks[blk].leases.push(lease);
    }
    cov('lov');
    if(typeof showToast==='function') showToast('✅ Lease added & synced','ok'); else alert('✅ Added!');
    rndrSolLand();
  }catch(err){
    if(typeof showToast==='function') showToast('❌ '+(err.message||'Save failed'),'er'); else alert(err.message||'Save failed');
  }
}
async function removeLease(bid,idx){
  if(!confirm('Remove?'))return;
  const blk = DB.solLand.blocks[bid];
  const lease = blk && blk.leases ? blk.leases[idx] : null;
  try{
    if (lease && lease._id) {
      // Firebase-backed lease — delete at source; the listener
      // removes it from every device within a second.
      await dataApi.removeSolLease(bid, lease._id);
    }
    // Local echo (also covers legacy seed leases that only exist in memory)
    if (blk && blk.leases) blk.leases.splice(idx,1);
    rndrSolLand();
    if(typeof showToast==='function') showToast('Lease removed','wn');
  }catch(err){
    if(typeof showToast==='function') showToast('❌ '+(err.message||'Delete failed'),'er');
  }
}
