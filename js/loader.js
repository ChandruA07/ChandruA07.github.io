'use strict';
/**
 * loader.js (v10) — Fetches HTML partials and boots the app.
 * Site opens directly to the dashboard. No login wall.
...
const PARTIALS = {
  ...
  'views-mount': [
    ...
    'views/view-pod.html','views/view-dpr.html','views/view-safety.html',
    'views/view-manpower.html','views/view-map.html',
  ],
...
async function bootApp(){
  ...
  try{
    ...
    const htmls = await Promise.all(
      ids.map(id => Promise.all(PARTIALS[id].map(fetchPartial)).then(p => p.join('\n')))
    );
    ids.forEach((id, i) => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = htmls[i];
    });
    ...
    // Edit-mode prompt: Enter submits
    const lp = document.getElementById('l-p');
    if(lp) lp.addEventListener('keydown', e => { if(e.key === 'Enter') editLock.submit(); });
    ...
    // Restore localStorage as fast offline fallback. Firebase listeners
    // overwrite this with cloud data once they fire.
    const restored = (typeof loadDB === 'function') ? loadDB() : false;
    if(restored) console.log('[loader] DB restored from localStorage');

    // Initialise edit-mode pill
    if (typeof editLock !== 'undefined') {
      const pill = document.getElementById('edit-mode-pill');
      if (pill) {
        const u = editLock.isUnlocked();
        pill.textContent = u ? '🔓 Edit mode' : '🔒 View mode';
        pill.style.color = u ? 'var(--ok,#3ddc84)' : 'var(--t3,#7a93b0)';
      }
    }
    ...
    document.body.innerHTML = `
      <div style="padding:40px;font-family:sans-serif;background:#07101f;color:#ff5252;min-height:100vh;">
        ...
        <p style="color:#4a6a8a;font-size:13px;margin-top:8px;">
          ...
          • Terminal: <code>npx serve .</code><br>
          • Terminal: <code>python3 -m http.server 8080</code>
        </p>
      </div>`;
  }
}
