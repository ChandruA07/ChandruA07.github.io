...
function rndrSolar(){
  ...
  if(g){
    ...
    // "Major Ongoing Activities" box removed per requirement — the ITC
    // card strip alone summarises the blocks; details live in each ITC.
    g.innerHTML = `
...
function rndrITC(id){
  ...
  // Fresh ITC render → keep date in sync with the session selection.
  if(typeof _solProgDate!=='undefined' && _solProgDate===null && typeof dataApi!=='undefined' && dataApi.setProgressDate) dataApi.setProgressDate(null);
  ...
  el.innerHTML=`
    ...
    ${ed ? `<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;background:var(--card2);border:1px solid var(--b1);border-radius:7px;padding:6px 10px;margin-bottom:10px;">
      ...
      <input type="date" id="sol-prog-date" max="${(typeof _progDateToday==='function')?_progDateToday():new Date().toISOString().slice(0,10)}"
             value="${(typeof _progDateToday==='function')?_progDateToday():new Date().toISOString().slice(0,10)}"
             onchange="solSetProgDate(this.value)"
    ...
    <div class="pnl" style="padding:12px 14px;border-radius:0 0 8px 8px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:start;">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;">
          ...
          <div id="sol-chart-${id}" style="width:100%;max-width:500px;">${_solRndrClusterChart(d, id)}</div>
        </div>
      </div>
...
function _solRndrClusterChart(itc, itcId){
  ...
  // Each sub-activity is one "bullet bar": a grey TOTAL track with faint
  // vertical gridlines, overlaid by a vibrant gradient DONE bar in a
  // rotating Power BI categorical palette, with the done value labelled
  // INSIDE the bar end, the total at the right, and a % pill per row.
  // A KPI header strip (donut + counters) summarises the activity.
  const PBI = ['#118DFF','#E66C37','#6B007B','#E044A7','#744EC2','#D9B300','#12239E','#D64550','#197278','#1AAB40'];

  const doneCount = subs.filter(s2 => (typeof solSubFrac==='function' ? solSubFrac(s2) : 0) >= 1).length;
  const wipCount  = subs.filter(s2 => { const f=(typeof solSubFrac==='function'?solSubFrac(s2):0); return f>0 && f<1; }).length;

  const rows = subs.map((s2,i) => {
    const done  = s2.doneQty || 0;
    const total = s2.totalQty || 0;
    const unit  = s2.unit || '';
    const donePct  = Math.min(100, (done  / niceMax) * 100);
    const totalPct = Math.min(100, (total / niceMax) * 100);
    const subFrac = (typeof solSubFrac === 'function') ? Math.round(solSubFrac(s2)*100) : 0;
    const col = subFrac >= 100 ? '#1AAB40' : PBI[i % PBI.length];
    const labelInside = donePct > 22;   // value label sits inside the bar when it fits
    return `<div style="margin-bottom:13px;">
      <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px;gap:8px;">
        <span style="font-size:9.5px;font-weight:700;color:var(--t1);
                     overflow:hidden;white-space:nowrap;text-overflow:ellipsis;flex:1;"
              title="${esc(s2.n)}">
          <span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:${col};margin-right:6px;vertical-align:0;"></span>${i+1}. ${esc(s2.n)}
        </span>
        <span style="flex-shrink:0;font-size:8.5px;font-weight:800;padding:2px 9px;border-radius:10px;
                     color:${subFrac>=100?'#1AAB40':subFrac>0?'#E66C37':'var(--t3)'};
                     background:${subFrac>=100?'rgba(26,171,64,.14)':subFrac>0?'rgba(230,108,55,.14)':'var(--card3)'};
                     border:1px solid ${subFrac>=100?'rgba(26,171,64,.4)':subFrac>0?'rgba(230,108,55,.4)':'var(--b1)'};">${subFrac}%</span>
      </div>
      <div style="position:relative;height:20px;border-radius:5px;background:var(--card3);
                  background-image:repeating-linear-gradient(90deg, var(--b1) 0 1px, transparent 1px calc(100%/5));
                  border:1px solid var(--b1);overflow:hidden;"
           title="${esc(s2.n)} — Done ${done}${unit?(' '+unit):''} of ${total}${unit?(' '+unit):''} (${subFrac}%)">
        <!-- total track -->
        <div style="position:absolute;inset:0;width:${totalPct}%;background:linear-gradient(180deg,rgba(154,160,170,.30),rgba(154,160,170,.14));border-radius:5px 0 0 5px;"></div>
        <!-- done bar -->
        <div style="position:absolute;top:2px;bottom:2px;left:2px;width:calc(${donePct}% - 2px);min-width:${done>0?'4px':'0'};
                    background:linear-gradient(180deg,${col},${col}cc 60%,${col}88);
                    border-radius:4px;box-shadow:0 0 8px ${col}55;transition:width .7s cubic-bezier(.22,.8,.36,1);
                    display:flex;align-items:center;justify-content:flex-end;">
          ${labelInside && done>0 ? `<span style="font-size:8.5px;font-weight:800;color:#fff;padding-right:6px;text-shadow:0 1px 2px rgba(0,0,0,.5);">${done}${unit?(' '+unit):''}</span>` : ''}
        </div>
        ${!labelInside && done>0 ? `<span style="position:absolute;left:calc(${donePct}% + 6px);top:50%;transform:translateY(-50%);font-size:8.5px;font-weight:800;color:${col};">${done}${unit?(' '+unit):''}</span>` : ''}
        <!-- total value, pinned right -->
        <span style="position:absolute;right:6px;top:50%;transform:translateY(-50%);font-size:8px;font-weight:700;color:var(--t3);">of ${total}${unit?(' '+unit):''}</span>
      </div>
    </div>`;
  }).join('');

  // X-axis gridline labels (aligned with the 5 background gridlines)
  const axis = `<div style="display:flex;justify-content:space-between;border-top:1px solid var(--b1);
              padding-top:4px;margin-top:2px;">
    ${gridLabels.map(v=>`<span style="font-size:7.5px;color:var(--t3);font-variant-numeric:tabular-nums;">${v}</span>`).join('')}
  </div>`;

  // CSS conic donut for overall activity % — no canvas needed
  const donut = `
    <div style="position:relative;width:62px;height:62px;flex-shrink:0;">
      <div style="position:absolute;inset:0;border-radius:50%;
                  background:conic-gradient(${def.color} ${actPct*3.6}deg, var(--b1) 0deg);"></div>
      <div style="position:absolute;inset:7px;border-radius:50%;background:var(--card);
                  display:flex;align-items:center;justify-content:center;flex-direction:column;">
        <span style="font-size:14px;font-weight:800;color:${def.color};line-height:1;">${actPct}%</span>
        <span style="font-size:6px;color:var(--t3);letter-spacing:.6px;">DONE</span>
      </div>
    </div>`;

  return `<div>
    <!-- Power BI-style header: donut + activity meta + KPI chips -->
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;padding:10px 12px;
                background:linear-gradient(135deg,var(--card2),var(--card3));
                border:1px solid var(--b1);border-left:4px solid ${def.color};border-radius:9px;">
      ${donut}
      <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:center;gap:6px;">
          <span style="font-size:14px;">${def.icon}</span>
          <div style="font-size:12px;font-weight:800;color:${def.color};
                      overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${esc(actDef.n)}</div>
        </div>
        <div style="font-size:8.5px;color:var(--t3);margin:3px 0 7px;">${esc(def.label)}</div>
        <div style="display:flex;gap:7px;flex-wrap:wrap;">
          <span style="font-size:8px;font-weight:700;color:var(--t2);background:var(--card);border:1px solid var(--b1);border-radius:9px;padding:2px 9px;">📦 ${subs.length} subs</span>
          <span style="font-size:8px;font-weight:700;color:#1AAB40;background:rgba(26,171,64,.12);border:1px solid rgba(26,171,64,.35);border-radius:9px;padding:2px 9px;">✓ ${doneCount} complete</span>
          <span style="font-size:8px;font-weight:700;color:#E66C37;background:rgba(230,108,55,.12);border:1px solid rgba(230,108,55,.35);border-radius:9px;padding:2px 9px;">⏳ ${wipCount} in progress</span>
        </div>
      </div>
      <button class="btn bts" style="font-size:8px;padding:3px 10px;align-self:flex-start;"
              onclick="solOpenActModal('${itcId}','${sel.section}','${sel.actKey}')">Edit ↗</button>
    </div>

    <!-- legend -->
    <div style="display:flex;gap:14px;justify-content:center;margin-bottom:9px;">
      <span style="display:flex;align-items:center;gap:5px;font-size:8.5px;color:var(--t2);font-weight:600;">
        <span style="width:18px;height:9px;background:linear-gradient(180deg,#118DFF,#118DFF88);border-radius:3px;display:inline-block;"></span>Done Qty (coloured per row)
      </span>
      <span style="display:flex;align-items:center;gap:5px;font-size:8.5px;color:var(--t2);font-weight:600;">
        <span style="width:18px;height:9px;background:linear-gradient(180deg,rgba(154,160,170,.30),rgba(154,160,170,.14));border:1px solid var(--b1);border-radius:3px;display:inline-block;"></span>Total Qty (track)
      </span>
    </div>
  ...
  const tiles = acts.map(a => {
    ...
    return `<div id="sol-tile-${itcId}-${sectionKey}-${a.key}"
      ...
      <div style="font-size:8.5px;font-weight:700;color:var(--t2);line-height:1.2;overflow:hidden;
                  display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;min-height:31px;margin-top:6px;">${esc(a.n)}</div>
...
function _renderActGrid(id){
  ...
  g.innerHTML=d.acts.map((a,i)=>`
    <div onclick="showSolActDetail('${id}',${i})"
      ...
      <div style="font-size:10px;font-weight:700;color:var(--t2);line-height:1.2;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;min-height:24px;">${esc(a.n)}</div>
...
function showSolActDetail(itcId,idx){
  ...
  if(pieBox){
    ...
    pieBox.innerHTML=`
      <div class="ph2" style="margin-bottom:8px;">
        <div class="pt" style="color:${a.col};">${esc(a.n)}</div>
  ...
  subPanel.innerHTML=`
    <div style="background:var(--card2);border:1px solid ${a.col};border-radius:8px;padding:10px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        <div style="font-family:var(--f2);font-size:11px;font-weight:700;color:${a.col};">${esc(a.n)} — Sub-Activities</div>
...
async function updateSolSubScope(itcId,actIdx,newScope){
  ...
  try {
    ...
    // subScope now has a first-class dataApi function (Supabase column)
    await dataApi.setSolarActSubScope(itcId, actIdx, Number(newScope) || 0).catch(()=>{});
  } catch (err) {
...
async function handleMapUpload(itcId,input){
  ...
  try {
    ...
    ITC_MAPS[itcId] = url;
    // Persist the map URL (module_state 'solar/itcMaps') so every device shows it.
    try { dataApi.setItcMap(itcId, url); } catch(e){}
...
function _solRndrSubActivity(itc, itcId, sectionKey, actKey, idx, meta){
  ...
  return `<div class="wtg-sub">
    <div class="wtg-sub-head">
      <span style="font-size:10px;font-weight:600;color:var(--t1);flex:1;">${idx+1}. ${esc(s.n)} ${lockIcon}${subDelBtn}</span>
...
function solOpenRowForm(itcId){
  ...
  const actOptGroups = ['pre','install','post'].map(sec => {
    ...
    const opts = solGetActivities(sec)
      .map(a => `<option value="${esc(sec)}::${esc(a.key)}">${esc(a.n)}</option>`).join('');
...
// Per-ITC back-date for Solar inline edits.
let _solProgDate = null;
function solSetProgDate(iso){
  _solProgDate = (iso && /^\d{4}-\d{2}-\d{2}$/.test(iso)) ? iso : null;
  if(typeof dataApi!=='undefined' && dataApi.setProgressDate) dataApi.setProgressDate(_solProgDate);
  if(typeof showToast==='function'){
    const today=(typeof _progDateToday==='function')?_progDateToday():new Date().toISOString().slice(0,10);
    showToast(_solProgDate&&_solProgDate!==today ? '📅 Recording edits for '+_solProgDate : '📅 Recording edits for today','wn');
  }
}
...
async function solUploadSubPhoto(itcId, path, inputEl){
  ...
  // Upload starts immediately on selection — Firebase Storage returns a
  // short downloadURL which is the ONLY thing written to the database.
  // (Base64 blobs are never stored — they bloat every realtime listener.)
  if(typeof showToast === 'function') showToast('⏫ Uploading photo…','ok');
  try{
    const { url } = await storage.uploadSubPhoto('solar', itcId, file, pct => {
      if(typeof showToast === 'function' && pct === 100) showToast('📷 Processing…','ok');
    });
    const itc = DB.solar.itcs[itcId]; if(!itc) return;
    if(typeof solInitActs === 'function') solInitActs(itc);
    const [section, actKey, idxStr] = path.split('|');
...
async function _solPersistItc(itcId, itc){
  try {
    if(typeof dataApi !== 'undefined' && typeof dataApi.setSolarItc === 'function'){
      ...
      console.warn('[solar] dataApi unavailable — write skipped');
